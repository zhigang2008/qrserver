...
BEGIN_MESSAGE_MAP(CServerMgrDlg, CDialogEx)
	...
	ON_COMMAND(ID_MENU_SERVER_START, &CServerMgrDlg::OnBnClickedButton11)
	ON_COMMAND(ID_MENU_SERVER_STOP, &CServerMgrDlg::OnBnClickedButton12)
	ON_COMMAND(ID_MENU_DATA_START, &CServerMgrDlg::OnBnClickedButton21)
	ON_COMMAND(ID_MENU_DATA_STOP, &CServerMgrDlg::OnBnClickedButton22)
END_MESSAGE_MAP()
...
void CServerMgrDlg::OnAppExit()
{
	...
	if(sService!=NULL)
	{
		::CloseHandle(sService);
	}
	if(dService!=NULL)
	{
		::CloseHandle(dService);
	}
	if(hSCM!=NULL)
	{
			::CloseHandle(hSCM);
...
void CServerMgrDlg::CheckRegister()
{
	 ...
	 if (ERROR_SUCCESS !=result)
	 {
		// MessageBox(_T("注册表打开失败"));
		 ...
		 TCHAR dir[_MAX_PATH];  
         DWORD len = _MAX_PATH;  
         if(reg.QueryStringValue(_T("installDir"),dir,&len)==ERROR_SUCCESS)
	     {
				 installDir=CString(dir);
		 }else{
...
{
	ShellExecute(NULL,_T("Open"),installDir+_T("\\server\\logs"),NULL,NULL,SW_SHOWNORMAL);
}
...
{
	ShellExecute(NULL,_T("Open"),installDir+_T("\\data\\logs"),NULL,NULL,SW_SHOWNORMAL);
}
...
void CServerMgrDlg::CheckService()
{
	 ...
	 if(sService!=NULL)
	 {
		 if(QueryServiceStatus(sService,&returnstatus))
	     {
	 ...
	 if(dService!=NULL)
	 {
	    if(QueryServiceStatus(dService,&returnstatus))
	    {
			if(returnstatus.dwCurrentState==SERVICE_RUNNING)
	     {
		   bDataStart=true;
...
void CServerMgrDlg::statusChange_ServerRun(void)
{
	...
	pMenuContext->EnableMenuItem(ID_MENU_SERVER_START,MF_GRAYED);//MF_BYCOMMAND | MF_DISABLED | MF_GRAYED
	pMenuContext->EnableMenuItem(ID_MENU_SERVER_STOP,MF_BYCOMMAND);
}
...
void CServerMgrDlg::statusChange_ServerStop(void)
{
	...
	GetDlgItem(IDC_BUTTON12)->EnableWindow(FALSE);
	pMenuContext->EnableMenuItem(ID_MENU_SERVER_START,MF_BYCOMMAND);//MF_BYCOMMAND | MF_DISABLED | MF_GRAYED
	pMenuContext->EnableMenuItem(ID_MENU_SERVER_STOP,MF_GRAYED);
}
...
void CServerMgrDlg::statusChange_DataRun(void)
{
	...
	pMenuContext->EnableMenuItem(ID_MENU_DATA_START,MF_GRAYED);
	pMenuContext->EnableMenuItem(ID_MENU_DATA_STOP,MF_BYCOMMAND);
}
...
void CServerMgrDlg::statusChange_DataStop(void)
{
	...
	GetDlgItem(IDC_BUTTON22)->EnableWindow(FALSE);
	pMenuContext->EnableMenuItem(ID_MENU_DATA_START,MF_BYCOMMAND);
	pMenuContext->EnableMenuItem(ID_MENU_DATA_STOP,MF_GRAYED);
}
